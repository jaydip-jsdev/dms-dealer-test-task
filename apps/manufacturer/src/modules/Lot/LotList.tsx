...
import { fetchLots } from "../../api/services/lotService";
import type { Lot } from "../../types";

import LotTable from "./LotTable/LotTable";
...
const LotList = () => {
  ...
  const loadData = useCallback(
    async (page: number, limit: number, search: string) => {
      setLoading(true);
      try {
        const response = await fetchLots({ page, limit, search });
  ...
  return (
    <Flex className="lot-container" vertical gap={20}>
      <Flex vertical className="lot-title">
        <Title level={5}>Lot List</Title>
        <Flex justify="space-between">
          ...
          <Button
            ...
            onClick={() => exportExcelCall("lot", "LotList", setExcelLoading)}
          >
            Export
          </Button>
        </Flex>
