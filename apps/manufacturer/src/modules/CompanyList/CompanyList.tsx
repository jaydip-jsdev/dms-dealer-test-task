...
import { fetchCompanies } from "../../api/services/companyService";
...
const CompanyList = () => {
  ...
  const loadData = useCallback(
    async (page: number, limit: number, search: string) => {
      setLoading(true);
      try {
        const response = await fetchCompanies({ page, limit, search });
  ...
  return (
    <Flex className="company-container" vertical gap={20}>
      <Flex vertical className="company-title">
        ...
        <Flex justify="space-between">
          ...
          <Button
            ...
            onClick={() =>
              exportExcelCall("company", "CompanyList", setExcelLoading)
            }
          >
            Export
          </Button>
